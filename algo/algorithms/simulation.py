...
from tools.consts import Direction, Motion
...
class MazeSolverSimulation:
    ...
    def generate_random_obstacles(self, num_obstacles: int):
        ...
        banned_gridsq = self._get_forbidden_area([(robot_state.x, robot_state.y)], padding=padding)
        obstacles = []
        for i in range(num_obstacles):
            ...
            direction = random.choice([Direction.NORTH, Direction.EAST, Direction.SOUTH, Direction.WEST])
        ...
        if self.debug:
            print(f"Debug mode enabled. Storing obstacles to file {os.path.realpath(self.debug_file)}")
    ...
    def plot_optimal_path_animation(self, verbose=False, testing=False):
        ...
        if testing:
            motions = self.maze_solver.optimal_path_to_motion_path(optimal_path)
            commands = CommandGenerator(straight_speed=60, turn_speed=40).generate_commands(motions)
            print()
        ...
        if verbose:
            ...
            for grid_sq in optimal_path:
                if grid_sq == optimal_path[-1]:
                    ...
                    print(f"({grid_sq.x}, {grid_sq.y}) {grid_sq.direction.name}", end=" ->  ")
        ...
        for cell in optimal_path:
            ...
            if abs(prev_cell.x - cell.x) > 1 or abs(prev_cell.y - cell.y) > 1:
                ...
                if angle is None:
                    ...
                    # half turn
                    angle = self._get_half_turn_angles(prev_cell.x, prev_cell.y, cell.x, cell.y, prev_cell.direction)

                angle_idx = markers_angle.index(angle)
                x_diff = cell.x - prev_cell.x
                y_diff = cell.y - prev_cell.y
                for i in range(1, 1 + num_points):
                    angle_x[angle_idx].append((prev_cell.x + i * x_diff / (1 + num_points), time))
                    angle_y[angle_idx].append((prev_cell.y + i * y_diff / (1 + num_points), time))
                    time += 1
        ...
        def update(frame_num):
            ...
            for j in range(len(markers)):
                ...
                if data_x[j] and data_y[j]:
                    ax.scatter(data_x[j], data_y[j], marker=markers[j], color='blue', s=80)

                if data_x_turn[j] and data_y_turn[j]:
                    ax.scatter(data_x_turn[j], data_y_turn[j], marker=(3, 0, markers_angle[j]), color='green', s=120)

        if verbose:
            print("Animating optimal path...")

        ani = animation.FuncAnimation(fig, update, frames=time + 5, interval=300)
        out_path = os.path.realpath(os.path.join(os.path.dirname(__file__), "../animations", "optimal_path.gif"))
