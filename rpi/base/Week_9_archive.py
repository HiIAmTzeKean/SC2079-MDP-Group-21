...
class RaspberryPi:
    ...
    def start(self):
        ...
        try:
            ...
            #logger.info(f"PREINFER small direction is: {self.small_direction}")
    ...
    def reconnect_android(self):
        ...
        while True:
            ...
            logger.error("Android link is down!")

            # Kill child processes
            logger.debug("Killing android child processes")
            ...
            logger.debug("Android child processes killed")
            ...
            logger.info("Android child processes restarted")
    ...
    def recv_android(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                logger.debug("Event set: Android connection dropped")
            ...
            if message['cat'] == "control":
                if message['value'] == "start":
                    ...
                    self.small_direction = self.snap_and_rec("Small")
                    logger.info(f"HERE small direction is: {self.small_direction}")
                    if self.small_direction == "Left Arrow": 
                        ...
                        logger.info("Acquiring near_flag log")
                        self.near_flag.acquire()             
                        
                        self.command_queue.put("OB01") # ack_count = 3
                        

                    logger.info("Start command received, starting robot on Week 9 task!")
    ...
    def recv_stm(self) -> None:
        ...
        while True:
            ...
            if message.startswith("ACK"):
                ...
                logger.debug(f"ACK from STM32 received, ACK count now:{self.ack_count}")
                
                
                logger.info(f"self.ack_count: {self.ack_count}")
                if self.ack_count == 3:
                    try:
                        self.near_flag.release()
                        logger.debug("First ACK received, robot reached first obstacle!")
                        ...
                        if self.small_direction == "Left Arrow": 
                            ...
                            self.command_queue.put("UL00") # ack_count = 5
                            logger.debug("Failed first one, going left by default!")
                    # except:
                        # logger.info("No need to release near_flag")
                    
                # if self.ack_count == 3:
                    except:
                        time.sleep(2)
                        logger.debug("First ACK received, robot finished first obstacle!")
                        ...
                        if self.large_direction == "Left Arrow": 
                            ...
                            self.command_queue.put("PR01") # ack_count = 6
                            logger.debug("Failed second one, going right by default!")

                if self.ack_count == 6:
                    logger.debug("Second ACK received from STM32!")
    ...
    def android_sender(self) -> None:
        while True:
            ...
            try:
                ...
                self.android_dropped.set()
                logger.debug("Event set: Android dropped")
    ...
    def snap_and_rec(self, obstacle_id: str) -> None:
        ...
        while True:
            ...
            if results['image_id'] != 'NA' or retry_count > 6:
                ...
                logger.info(f"Image recognition results: {results}")
                logger.info("Recapturing with same shutter speed...")
            elif retry_count <= 4:
                logger.info(f"Image recognition results: {results}")
                logger.info("Recapturing with lower shutter speed...")
                speed -= 1
            elif retry_count == 5:
                logger.info(f"Image recognition results: {results}")
                logger.info("Recapturing with lower shutter speed...")
        ...
        logger.info(f"Image recognition results: {results} ({ans})")
        return ans
    ...
    def request_stitch(self):
        ...
        if response.status_code != 200:
            logger.error("Something went wrong when requesting stitch from the API.")
            return
